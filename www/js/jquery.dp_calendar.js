...
(function ($) {
	$.fn.dp_calendar = function (options) {	
		...
		show_end_time = opts.show_end_time;
		...
		if(json != "" && json != null) {
			...
			$.ajax({
				...
				success: function(data) {
					...
					if(typeof parsedJson.time_format !== "undefined") {
						time_format = parsedJson.time_format;
					}
					...
					if(typeof parsedJson.show_end_time !== "undefined") {
						show_end_time = parsedJson.show_end_time;
					}
		...
		function calculeDates() {
			...
			$.fn.dp_calendar.time_format = time_format;
				...
				if(typeof(this) == "object") {
					...
					if((Date.UTC(date_selected.getFullYear(), date_selected.getMonth(), date_selected.getDate()) === Date.UTC(startDate.getFullYear(), startDate.getMonth(), startDate.getDate())) || recurring_frecuency_active){
						...
						if(this["allDay"]) {
							...
							if(!format_ampm) {
									$(li_event_time).html(startDate.toString(time_format) + (show_end_time ? " - " + endDate.toString(time_format) : ''));
							} else {
									$(li_event_time).html(startDate.toString(time_format.replace("HH", "h").replace("H", "h")+" tt") + (show_end_time ? " - " + endDate.toString(time_format.replace("HH", "h").replace("H", "h")+" tt") : ''));
							}
						}
	...
	$.fn.dp_calendar.time_format = $.fn.dp_calendar.defaults.time_format;
